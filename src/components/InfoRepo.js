...
const InfoRepo = (props) => {
    ...
    if (newLink) {
        ...
        for (let i = 0 ; i < menu.length; i++){
            if(i===0){
                owner = menu[i].split(/\s+/).map(word => word[0].toUpperCase() + word.substring(1)).join(' ')
                ownerLink = `https://github.com/${owner}`
                owner += ' ' + '>'
            } else {
                repo = menu[i].split(/\s+/).map(word => word[0].toUpperCase() + word.substring(1)).join(' ') 
            }
        } 
    }
  ...
  return (
    <React.Fragment>
        <div className={classes.containers}>
            <a className={classes.link} href={ownerLink}>{owner}</a> 
            <a className={classes.link} href={newLink}>{repo}</a> 
